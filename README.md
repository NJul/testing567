...
## Допустим, вы сделали commit в git, но поняли, что он недостаточно хорош. В таком случае можно продолжить правки, а при следующем коммите набрать: Ключ --amend (улучшить, в переводе с английского) позволяет добавить к последнему коммиту новые изменения.
...
## В случае, если вы уже опубликовали свои изменения, то не остается ничего другого, как сделать коммит, который отменяет какой-то коммит:

git revert commit-sha1

## Если вы хотите "отменить", все незафиксированные изменения просто запускаете:
